...
If you need access to the expanded state of the rows in other parts of your application, you can own the `expanded` state slice yourself. The recommended way in v9 is an external atom passed through the `atoms` table option. Atoms preserve fine-grained subscriptions, and the expanded value can be read or subscribed to from any module without going through the component that owns the table.

```ts
import { createAtom } from '@tanstack/store'
import type { ExpandedState } from '@tanstack/lit-table'
...
// read expandedAtom.get() (or subscribe to expandedAtom) wherever you need the value
```
...
const table = this.tableController.table({
  ...
  onExpandedChange: (updater) => {
    this.expanded = typeof updater === 'function' ? updater(this.expanded) : updater
  },
})
```
