---
title: Custom Features (React) Guide
---
...
Now that the feature is added to the table instance, you can use the new instance APIs, options, and state in your application. The [custom-plugin example](../examples/custom-plugin) controls the `density` state with `React.useState` and the new `onDensityChange` option:
