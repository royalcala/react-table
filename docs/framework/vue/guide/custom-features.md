...
Check out the full custom-plugin example (currently available for React) to see the full implementation, but here's an in-depth look at the steps to create a custom feature.
...
Now that the feature is added to the table instance, you can use the new instance APIs, options, and state in your application. For example, you can control the `density` state with a Vue ref and the new `onDensityChange` option:

```ts
const features = tableFeatures({ densityPlugin })

const density = ref<DensityState>('md')
...
const table = useTable({
  ...
  state: {
    get density() {
      return density.value // passing the density state to the table, TS is still happy :)
    },
  },
  onDensityChange: (updater) => {
    density.value = updater instanceof Function ? updater(density.value) : updater
  },
})
```
...
<td
  :style="{
    padding: table.atoms.density.get() === 'sm'
      ? '4px'
      : table.atoms.density.get() === 'md'
        ? '8px'
        : '16px',
...
Reads of `table.atoms.density.get()` are reactive inside Vue templates, so the cells re-render whenever the density state changes.

#### Do We Have to Do It This Way?

This is just a new way to integrate custom code along-side the built-in features in TanStack Table. In our example up above, we could have just as easily stored the `density` state in a `ref`, defined our own `toggleDensity` handler wherever, and just used it in our code separately from the table instance. Building table features along-side TanStack Table instead of deeply integrating them into the table instance is still a perfectly valid way to build custom features. Depending on your use case, this may or may not be the cleanest way to extend TanStack Table with custom features.
