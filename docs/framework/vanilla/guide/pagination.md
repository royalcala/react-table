---
title: Pagination (Vanilla JS) Guide
---
...
Vanilla examples use the core table package directly with store reactivity bindings and imperative DOM rendering.
...
const setPagination = (updater) => {
  ...
  table.setOptions((prev) => ({ ...prev, state: { ...prev.state, pagination } }))
}
...
previousPageButton.disabled = !table.getCanPreviousPage()
previousPageButton.onclick = () => table.previousPage()
...
pageSizeSelect.value = String(table.store.state.pagination.pageSize)
