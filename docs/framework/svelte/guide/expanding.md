...
```ts
import { createTable, tableFeatures, rowExpandingFeature, createExpandedRowModel } from '@tanstack/svelte-table'
...
## Expanding Feature (Svelte) Guide
...
const expanded = useSelector(expandedAtom) // read it reactively with expanded.current
